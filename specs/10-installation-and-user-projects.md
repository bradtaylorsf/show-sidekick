...
Show Sidekick is installed as a CLI. The user runs the CLI inside their **own** folder, which is owned by them — not by Show Sidekick. This separates the harness (versioned, public, Apache 2.0) from user content (their shows, their characters, their music, their license).

```
┌────────────────────────────────┐         ┌────────────────────────────────┐
│  Show Sidekick  (this repo)    │         │  <user-project>  (their repo)  │
│  ─────────────────────         │         │  ─────────────────────────     │
...
```bash
cd ~/my-shows
showkick init                       # creates CLAUDE.md, AGENTS.md, .env.example, .env, .show-sidekick/, .gitignore
showkick init --git                 # same, plus `git init`
showkick init --starter animated-explainer # scaffold the first-video starter show alongside the project
showkick init --no-setup-runtimes   # skip default Remotion, Remotion CLI deps, and HyperFrames install
```
...
├── .gitignore               # excludes generated media, .show-sidekick/, .env
├── .env                     # local provider keys, copied from .env.example, gitignored
├── .env.example             # committed blank provider/tool setup template
├── .show-sidekick/                 # local cache of bundled harness content (gitignored)
│   ├── version.json
│   ├── pipelines/
│   ├── playbooks/
│   ├── skills/
...
Non-`init` commands detect the user project root from `CLAUDE.md` plus either an existing `.show-sidekick/` cache or the committed scaffold pair `AGENTS.md` + `.env.example`. This lets a freshly cloned user project work even though `.show-sidekick/` is gitignored. Before a command runs, the CLI restores or refreshes `.show-sidekick/` automatically when the cache is missing, locked to a compatible older harness, or has a stale bundled checksum.

`showkick update` refreshes `.show-sidekick/` from the currently installed harness version on demand.

`.show-sidekick/version.json` records the cache lock:
...
`showkick update --check` compares the installed harness version and bundled checksum against that file without writing. It exits non-zero when the cache is stale. Commands still refuse to run when the cache was locked by an incompatible major version; the remediation is either `showkick update` to refresh the project cache intentionally or installing a matching Show Sidekick version.

`show-sidekick.lock` is a post-v0.1.0 team-pinning feature. v0.1.0 does not write or enforce `show-sidekick.lock`; cache compatibility is tracked only by `.show-sidekick/version.json`. When implemented, `showkick init` will write the lock with harness version and bundled checksum, `showkick update` will error on lock mismatch unless forced, and `showkick update --check` will remain non-mutating and exit non-zero on mismatch.

### `showkick new show <slug> [--from <starter>]`

Creates `shows/<slug>/`. With `--from <starter>`, copies a starter template from `.show-sidekick/starters/<starter>/` as the initial content. Starters carry an example `show.yaml`, a brand stub, sample characters, and an episode template.
...
Available starters listed by `showkick ls starters`.
...
| Pipelines | `harness/pipelines/` → cached in `.show-sidekick/pipelines/` | Yes — override by placing same-named file in `<project>/pipelines/` |
| Playbooks | `harness/playbooks/` → cached in `.show-sidekick/playbooks/` | Yes — override in `<project>/playbooks/` |
...
| Starters | `harness/starters/` → cached | No (cloned via `showkick new show --from`, then user-edited) |
...
2. Fall back to the cache (`<project>/.show-sidekick/pipelines/<name>.yaml`).
3. Error if neither exists.

User overrides always win. The bundled cache is read-only from the harness's perspective — `showkick update` is the only thing that writes to it.

## Maintainer validation outside the harness repo

The demo matrix runner validates the installed/local CLI from the same separation boundary users see. `pnpm demo-matrix` creates temp user projects outside the harness repo, runs `showkick init --starter <slug>` in each lane project, then runs `showkick build <show>/sample-episode --sample`. Generated renders, checkpoints, cost logs, and decisions stay under the temp user project, never under the harness source tree. Use `--keep-workdir` when those artifacts need manual inspection after a failed lane.

## Why a local cache instead of reading from `node_modules`

- Agents (Claude Code, Codex) navigate via filesystem reads. `.show-sidekick/skills/...` is a stable, predictable path inside the project. `node_modules/show-sidekick/...` is brittle (depends on install method, pnpm hoisting, global vs local).
- The cache is gitignored, so a fresh clone of the user's project doesn't carry stale harness content; the next non-`init` command restores it from whatever harness version is installed, and `showkick update` remains the explicit refresh command.
- The cache version is tracked in `.show-sidekick/version.json` so the harness can warn on mismatch and refuse to operate on a project cached against an incompatible major version.

## User project is user-owned

The user picks the license, the source-control host, the backup strategy, and what to publish. The harness has no opinion on whether their `shows/` directory is a public GitHub repo or a personal Backblaze backup. Show Sidekick produces video; it does not manage user content.

## Multi-user / team scenarios

A team can share a user project the same way they share any git repo. The cached `.show-sidekick/` is gitignored — each contributor's `.show-sidekick/` reflects their locally installed harness version. Recommend pinning the harness version in a `show-sidekick.lock` file (future feature) so all collaborators stay in sync.
