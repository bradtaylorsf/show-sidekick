...
const hasFfmpeg = hasBinary("ffmpeg") && hasFfprobe;
let tempDirs: string[] = [];
...
describe("ffprobe", () => {
  it.skipIf(!hasFfprobe)("returns normalized audio metadata for fixture media", async () => {
    const result = await ffprobe(fixture);
    const audioStream = result.streams.find((stream) => stream.codec_type === "audio");

    expect(result.format.duration_s).toBeGreaterThan(3.9);
    expect(audioStream).toBeDefined();
    expect(audioStream?.sample_rate).toBe(44_100);
  });
