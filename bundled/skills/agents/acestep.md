...
Open-source music generation (MIT license) via `src/tools/music_gen.py`. Runs on RunPod serverless.
...
```bash
# Basic generation
python src/tools/music_gen.py --prompt "Upbeat tech corporate" --duration 60 --output bg.mp3

# With musical control
python src/tools/music_gen.py --prompt "Calm ambient piano" --duration 30 --bpm 72 --key "D Major" --output ambient.mp3

# Scene presets (video production)
python src/tools/music_gen.py --preset corporate-bg --duration 60 --output bg.mp3
python src/tools/music_gen.py --preset tension --duration 20 --output problem.mp3
python src/tools/music_gen.py --preset cta --brand digital-samba --duration 15 --output cta.mp3

# Vocals with lyrics
python src/tools/music_gen.py --prompt "Indie pop jingle" --lyrics "[verse]\nBuild it better\nShip it faster" --duration 30 --output jingle.mp3

# Cover / style transfer
python src/tools/music_gen.py --cover --reference theme.mp3 --prompt "Jazz piano version" --duration 60 --output jazz_cover.mp3

# Stem extraction
python src/tools/music_gen.py --extract vocals --input mixed.mp3 --output vocals.mp3

# List presets
python src/tools/music_gen.py --list-presets
```
...
```bash
python src/tools/music_gen.py --prompt "Upbeat indie rock, driving drums, jangly guitar" --duration 60 --bpm 120 --key "G Major" --output track.mp3
```
...
```bash
python src/tools/music_gen.py --preset tension --duration 20 --output problem_scene.mp3
```
...
- **Sound effects** — use ElevenLabs SFX (`src/tools/sfx.py`)
