...
For deck-led `presentation-demo` episodes, voiceover is always the master clock. The deck supplies evidence and visual structure, but timing starts only after the script is approved and narration is generated or attached. Slide count never becomes the timing grid.
...
| Presentation demo | `['word']` plus slide-aware section boundaries — visuals follow VO meaning and reference `deck_manifest` slide IDs |
