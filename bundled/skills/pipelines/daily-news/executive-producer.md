...
topic + sources + recency window, fetch headlines, lock the story slate, script
narration, capture source pages, and produce a 5-15 min broadcast-style episode.

## Pipeline state machine

`research → idea → script → capture → scene_plan → assets → edit → compose → publish`

Read the corresponding director skill (`bundled/skills/pipelines/daily-news/<stage>-director.md`)
before starting each stage. This repo keeps `research` before `idea` to satisfy
the canonical stage order; the research stage uses episode/show defaults to
build candidates, then the idea stage locks the selected slate.
...
- **Reuse existing tools.** This pipeline intentionally does NOT have custom
  ...
  `web_search` for headlines, `playwright_recording` for source-page captures,
  and `video_downloader` only when a selected story requires retrieving an
  original source video. Follow `bundled/skills/agents/playwright-recording.md`
  and `bundled/skills/agents/video-download.md` when those paths are needed.
...
- **Idea:** confirm scope, sources, recency window, story slate, voice id,
  platform, and runtime (HEAVY — user picks which 5-10 of the 8-12 candidates
  to include).
...
The capture / scene_plan / assets / edit stages auto-proceed.
