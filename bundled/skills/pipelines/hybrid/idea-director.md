...
Before locking the production plan, decide `render_runtime` with the user. Hybrid supports BOTH Remotion and HyperFrames; neither is an auto-default. Follow the contract in specs/15-announce-and-escalate.md → "Present Both Composition Runtimes (HARD RULE)":

1. Check registry/preflight availability for `remotion`, `hyperframes`, and `ffmpeg` when the brief permits a simple stitch/export path. If both `remotion` and `hyperframes` are available, present both to the user with brief-specific analysis:
...
- `bundled/skills/agents/video-edit.md`
- `bundled/skills/meta/source-media-review.md`
