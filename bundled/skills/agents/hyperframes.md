...
2. **visual-style.md exists?** → Read it. Apply its `style_prompt_full` and structured fields. (Note: `visual-style.md` is a project-specific file. `hyperframes/visual-styles.md` is the style library with 8 named presets — different files.)
3. **User named a style** (e.g., "Swiss Pulse", "dark and techy", "luxury brand")? → Read [hyperframes/visual-styles.md](./visual-styles.md) for the 8 named presets. Generate a minimal DESIGN.md with: `## Style Prompt` (one paragraph), `## Colors` (3-5 hex values with roles), `## Typography` (1-2 font families), `## What NOT to Do` (3-5 anti-patterns).
...
For motion defaults, sizing, entrance patterns, and easing — follow [hyperframes/house-style.md](./house-style.md). The house style handles HOW things move. The DESIGN.md handles WHAT things look like.
...
When no `visual-style.md` or animation direction is provided, follow [hyperframes/house-style.md](./house-style.md) for aesthetic defaults.
...
- **[hyperframes/references/captions.md](hyperframes/references/captions.md)** — Captions, subtitles, lyrics, karaoke synced to audio. Tone-adaptive style detection, per-word styling, text overflow prevention, caption exit guarantees, word grouping. Read when adding any text synced to audio timing.
- **[hyperframes/references/tts.md](hyperframes/references/tts.md)** — Text-to-speech with Kokoro-82M. Voice selection, speed tuning, TTS+captions workflow. Read when generating narration or voiceover.
- **[hyperframes/references/audio-reactive.md](hyperframes/references/audio-reactive.md)** — Audio-reactive animation: map frequency bands and amplitude to GSAP properties. Read when visuals should respond to music, voice, or sound.
- **[hyperframes/references/css-hyperframes/patterns.md](hyperframes/references/css-hyperframes/patterns.md)** — CSS+GSAP marker highlighting: highlight, circle, burst, scribble, sketchout. Deterministic, fully seekable. Read when adding visual emphasis to text.
- **[hyperframes/references/typography.md](hyperframes/references/typography.md)** — Typography: font pairing, OpenType features, dark-background adjustments, font discovery script. **Always read** — every composition has text.
- **[hyperframes/references/motion-principles.md](hyperframes/references/motion-principles.md)** — Motion design principles: easing as emotion, timing as weight, choreography as hierarchy, scene pacing, ambient motion, anti-patterns. Read when choreographing GSAP animations.
- **[hyperframes/visual-styles.md](hyperframes/visual-styles.md)** — 8 named visual styles (Swiss Pulse, Velvet Standard, Deconstructed, Maximalist Type, Data Drift, Soft Signal, Folk Frequency, Shadow Cut) with hex palettes, GSAP easing signatures, and shader pairings. Read when user names a style or when generating DESIGN.md.
- **[hyperframes/house-style.md](hyperframes/house-style.md)** — Default motion, sizing, and color palettes when no style is specified.
- **[hyperframes/patterns.md](hyperframes/patterns.md)** — PiP, title cards, slide show patterns.
- **[hyperframes/data-in-motion.md](hyperframes/data-in-motion.md)** — Data, stats, and infographic patterns.
- **[hyperframes/references/transcript-guide.md](hyperframes/references/transcript-guide.md)** — Transcription commands, whisper models, external APIs, troubleshooting.
- **[hyperframes/references/dynamic-techniques.md](hyperframes/references/dynamic-techniques.md)** — Dynamic caption animation techniques (karaoke, clip-path, slam, scatter, elastic, 3D).

- **[hyperframes/references/transitions.md](hyperframes/references/transitions.md)** — Scene transitions: crossfades, wipes, reveals, shader transitions. Energy/mood selection, CSS vs WebGL guidance. **Always read for multi-scene compositions** — scenes without transitions feel like jump cuts.
  - [transitions/catalog.md](hyperframes/references/transitions/catalog.md) — Hard rules, scene template, and routing to per-type implementation code.
