...
8. For a zero-key personalized first video, use the `animated-explainer` starter and rewrite `shows/<show>/inputs/<episode>/script.txt` into four concise narrated card lines: a tailored hook, one personal-use beat, one workflow beat, and the next step. Keep `duration_s: 30`. Then run `predit build <show>/<episode> --sample`.
