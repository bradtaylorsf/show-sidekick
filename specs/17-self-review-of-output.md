...
```ts
{
  ...
  checks: {
    ...
    visual_spotcheck: { frames_sampled: number, findings: [...] },
    audio_spotcheck: { /* ... */ },
    ...
    subtitle_check: { present: boolean, accuracy_within_150ms: number /* percent */ }
  },
  issues_found: [/* aggregated findings */],
  recommended_action: 'present_to_user' | 'rerender' | 'investigate'
}
```
...
- **fail** → the harness halts. The user is shown the issues and asked how to proceed. The pipeline **must not** advance to publish or hand-off stages on a failing self-review.
