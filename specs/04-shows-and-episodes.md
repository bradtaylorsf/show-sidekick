...
For `caption_burn` recipes, `props.source: cuesheet` resolves from the current run's cuesheet artifact, then prior checkpoints, then an explicit runner cuesheet, then `projects/<show>/<episode>/cuesheet.json` if it was generated by `showkick cuesheet`. If none exists, compose surfaces a recipe validation error instead of inventing timings.
