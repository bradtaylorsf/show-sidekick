...
### At compose stage:
1. The pre-compose validation in `src/compose/pre-compose-validation.ts` enforces this automatically.
...
1. **Existence**: If user-supplied files were provided to the project, does a `source_media_review` artifact exist?
   - If user media exists but no `source_media_review`: **CRITICAL** — "User supplied media but the agent did not inspect it before planning. Run the `source_media_review` tool before proceeding."
