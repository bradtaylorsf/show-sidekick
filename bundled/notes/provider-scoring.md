...
`predit` v0.1 keeps tool routing intentionally simple:
...
This deliberately differs from the sibling-of-record provider selector, which used a seven-dimension weighted formula. The known consequence is that `predit` may choose a different provider for the same brief when multiple providers are technically valid.
