...
Reusable video components in `src/remotion/components/`. Import in templates via:

```tsx
import { AnimatedBackground, SlideTransition, Label } from '../../../../src/remotion/components';
```
...
The toolkit includes a transitions library at `src/remotion/transitions/` for scene-to-scene effects beyond the official `@remotion/transitions` package.
...
import { glitch, lightLeak, clockWipe, checkerboard } from '../../../../src/remotion/transitions';
...
For detailed API documentation on all hooks, components, renderer, Lambda, and Player APIs, see [remotion/reference.md](remotion/reference.md).
