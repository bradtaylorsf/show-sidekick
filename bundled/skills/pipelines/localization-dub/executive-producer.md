...
The only pre-planning stage is `source_review`: inspect the existing source video before localization scope, translation, or voice casting decisions are locked. The EP manages the complexity of producing multiple language variants while preserving the original's timing and quality.

## Prerequisites

| Layer | Resource | Purpose |
|-------|----------|---------|
| Pipeline | `bundled/pipelines/localization-dub.yaml` | Stage definitions |
...
```
CHECK: Source video grounding
  - Was the source video probed before translation decisions?
  - Are speech-bearing sections and visible-mouth shots identified?
  - Are on-screen text and baked-in subtitle risks mapped?
```
...
|------|-------------|---------------|-------------|
| G0 | source_review | Source probe, speech/on-screen text risks | Revise |
