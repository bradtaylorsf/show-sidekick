...
The only pre-planning stage is `source_review`: inspect supplied source material so the rest of the pipeline stays grounded. Do not expand this into speculative research unless the user asks. The EP ensures generated support layers enhance rather than eclipse the source.

## Prerequisites

| Layer | Resource | Purpose |
|-------|----------|---------|
| Pipeline | `bundled/pipelines/hybrid.yaml` | Stage definitions |
| Skills | All 8 director skills + `bundled/skills/meta/reviewer.md` | Stage execution |
...
### After SOURCE_REVIEW stage:
```
CHECK: Source inventory grounding
  - Were all supplied media files reviewed or explicitly marked out of scope?
  - Does each summary cite concrete probe data?
  - Are source constraints and reusable source moments ready for idea and script?
```
...
|------|-------------|---------------|-------------|
| G0 | source_review | Source inventory, probe grounding, usable moments | Revise |
