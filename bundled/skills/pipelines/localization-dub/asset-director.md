...
| Prior artifacts | `priorArtifacts.scene_plan`, `priorArtifacts.script`, `priorArtifacts.idea` | Language plan and transcript package |
| Tools | `tts_selector`, `subtitle_gen`, `heygen_video`, `lip_sync`, `audio_enhance` — `tts_selector` auto-discovers all available TTS providers from the registry | Dubbed audio, subtitle, optional HeyGen video translation, and optional lip-sync production |
...
When the script package selects HeyGen video-translate, use `bundled/skills/agents/video-translate.md` and call the registry-backed `heygen_video` tool with `mode: "video_translate"`. Preserve the target language, `voice_id`, and review status in the asset metadata.
...
2. **Check `.show-sidekick/skills/agents/` and `bundled/skills/agents/`** for existing Layer 3 knowledge (provider-specific prompting guides, API patterns)
