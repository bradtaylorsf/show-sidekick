...
`predit export <show>/<episode> --target premiere|davinci` writes a Final Cut Pro 7 XML handoff package under `exports/` by default. Use `--asset-link-mode copy|symlink|reference` to control asset packaging and `--out <dir>` to choose a different export root. Each export writes `projects/<show>/<episode>/publish_log.json`.
