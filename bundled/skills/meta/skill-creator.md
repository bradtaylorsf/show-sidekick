...
|------|-----------|---------|
| Stage director | `bundled/skills/pipelines/<pipeline>/` or project `skills/pipelines/<pipeline>/` | How to execute a specific pipeline stage |
| Meta skill | `bundled/skills/meta/` or project `skills/meta/` | Cross-cutting protocol (review, checkpoint, onboarding, etc.) |
| Core craft skill | `bundled/skills/core/` | Cross-cutting craft technique such as FFmpeg, Remotion, subtitles, color grading |
| Vendor / Layer 3 skill | `bundled/skills/agents/` or project `skills/agents/` | Provider-specific prompt engineering, parameter defaults, and quality techniques |
...
1. Place it in the correct bundled or project-local skill directory.
2. If it's a pipeline stage skill, ensure the pipeline manifest references it in the stage's `skill` field.
3. If it's a vendor / Layer 3 skill, ensure tools or director skills reference it by name before generation.
4. If it changes harness behavior, update the relevant spec or README in the same change.
