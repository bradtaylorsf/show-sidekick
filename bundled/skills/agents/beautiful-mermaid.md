...
If the user provides a description rather than code, generate valid Mermaid syntax. Consult `beautiful-mermaid/references/mermaid-syntax.md` for full syntax details.
...
```bash
bun run beautiful-mermaid/scripts/render.ts --code "graph TD; A-->B" --output diagram --theme default
```

Or from a file:

```bash
bun run beautiful-mermaid/scripts/render.ts --input diagram.mmd --output diagram --theme tokyo-night
```

Alternative runtimes:
```bash
npx tsx beautiful-mermaid/scripts/render.ts --code "..." --output diagram
deno run --allow-read --allow-write --allow-net beautiful-mermaid/scripts/render.ts --code "..." --output diagram
```
...
```bash
bun run beautiful-mermaid/scripts/create-html.ts --svg diagram.svg --output diagram.html
```
