...
python3 src/tools/verify_setup.py
```
...
```bash
cd ~/.openclaw/workspace/claude-code-video-toolkit
pip3 install --break-system-packages -r src/tools/requirements.txt
```
...
```bash
cd ~/.openclaw/workspace/claude-code-video-toolkit
python3 src/tools/qwen3_tts.py --text "Hello, this is a test." --speaker Ryan --tone warm --output /tmp/video-toolkit-test.mp3 --cloud modal
```

If you get a valid .mp3 file, setup is complete. If it fails, check:
- `.env` has the correct `MODAL_QWEN3_TTS_ENDPOINT_URL`
- Run `python3 src/tools/verify_setup.py --json` and check `modal_tools` for which endpoints are missing
...
```bash
cd ~/.openclaw/workspace/claude-code-video-toolkit
cp -r video_toolkit/templates/product-demo projects/PROJECT_NAME
...
```bash
cd ~/.openclaw/workspace/claude-code-video-toolkit
python3 src/tools/music_gen.py \
  --preset corporate-bg \
  --duration 90 \
...
# Scene 01
python3 src/tools/qwen3_tts.py \
...
python3 src/tools/qwen3_tts.py \
  --text "Text to speak" \
  --ref-audio video_toolkit/assets/voices/reference.m4a \
...
python3 src/tools/sadtalker.py \
...
```bash
cd ~/.openclaw/workspace/claude-code-video-toolkit
python3 src/tools/image_edit.py \
  --input projects/PROJECT_NAME/public/images/title-bg.png \
...
import { glitch } from '../../../src/remotion/transitions/presentations/glitch';
import { lightLeak } from '../../../src/remotion/transitions/presentations/light-leak';
```

**NEVER import from `src/remotion/transitions` barrel** — import custom transitions from `src/remotion/transitions/presentations/` directly.
...
| Problem | Solution |
|---------|----------|
| Tool command fails with "No module named..." | Run `pip3 install --break-system-packages -r src/tools/requirements.txt` from toolkit root |
| "MODAL_*_ENDPOINT_URL not configured" | Check `.env` has the endpoint URL. Run `python3 src/tools/verify_setup.py` |
