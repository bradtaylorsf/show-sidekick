...
| Scenario | Prefer | Why |
|----------|--------|-----|
| Existing explainer, React scene component stack (text_card, stat_card, chart scenes, caption overlay, TalkingHead, CinematicRenderer) | **Remotion** | These compositions already exist in `src/remotion/`. Reusing them is free; replicating them in HTML is not. |
...
See `specs/15-announce-and-escalate.md` → "Present Both Composition Runtimes (HARD RULE)" for
...
per `specs/15-announce-and-escalate.md` > "Escalate Blockers Explicitly" and wait for user
...
- Existing documentary-montage end-tag overlay stack (relies on specific
  Remotion components)
- Anything that assumes assets are staged under `src/remotion/public/`
...
`src/remotion/public/` — that's Remotion's shared staging directory and
...
install instructions. Per `specs/15-announce-and-escalate.md` Setup Offer Protocol, group the
fix by effort:
...
See `src/compose/hyperframes-style-bridge.ts`. Playbooks do NOT need to fork — the
...
Chrome + FFmpeg). Track via `cost tracker`:
...
workspace. See `hyperframes`'s existing workspace prep — it
